      ...
      <view class="table_box">
                    ...
                    <template v-for="(tdItem, tdItemIndex) in columns">
                      ...
                      >
                        <view
                          class="td_wrap"
                          :class="[
                            lastBgGrey && index + 1 === list.length
                              ? 'lastBgGrey'
                              : '',
                          ]"
                          :style="{
                        ...
                        >
                          ...
                          <template v-if="tdItem.$operateList">
                            <template v-for="btn in tdItem.$operateList">
                              <view
                                v-if="btn?.isShow ? btn.isShow(item) : true"
                          ...
                          <template v-if="tdItem?.type == 'photo'">
                            <view
                              style="
                                display: flex;
                                align-items: center;
                            ...
                            >
                              <img
                                v-if="item[tdItem.key]"
                                :src="item[tdItem.key]"
                                alt="图片加载中..."
                                class="boss-table-img"
                              />

                              <text style="line-height: 45px" v-else>无</text>
                            </view>
                          </template>
                          <template v-else>
                            {{
                              tdItem.formate
                                ? tdItem.formate(item[tdItem.key], index, item)
                                : item[tdItem.key]
                                  ? item[tdItem.key]
                                  : ''
                            }}
        ...
        <template v-else>
          <view
            ...
            :style="{ height: '80px', width: emptyColWidth }"
          ></view>
        </template>
        <template v-if="list.length">
          <!-- 固定左边一列 【-->
...
export default {
  ...
  computed: {
    ...
    //没数据时候，主体的宽度
    emptyColWidth() {
      const t = this.columns.reduce((sum, currentValue) => {
        return sum + parseFloat(currentValue.$width)
      }, 0)
      return t + 'px'
    },
  },
